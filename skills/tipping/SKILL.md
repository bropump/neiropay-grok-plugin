...
Prepare the requested recipient, exact token, amount and units with `prepare_tip`. Generate a unique requestKey for the user's tip and keep it for retries. Check that the returned review matches the instruction. When deliveryMode is at-least, explain that the recipient gets the requested minimum plus any swap surplus within the quoted SOL cap. Show the review’s costBreakdown: token purchase, service/network/delivery fees, nonzero sender deposits and recipient funding, temporary deposits returned in the transaction, maximum SOL debit after refunds, and upfront SOL required. Do not describe rent deposits as network fees or summarize only the service fee. Obtain clarification if the amount, asset or recipient differs or is ambiguous. `send_tip` moves funds; invoke only for the user-authorized prepared tip. Do not create new spending limits or alter NeiroPay's rules and fees.
